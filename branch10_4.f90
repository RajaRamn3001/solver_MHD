...
contains
  subroutine csr_to_csc(csr_array,csc_array,col_num)
    ...
    allocate(csc_array%coeffs(nz_size),csc_array%row(nz_size))
     ...
     print*,"NNZ of csc_array",csc_array%nz_size
     print*,"NNZ of csr_array",nz_size
     open(unit=50, file='../output_file/node_info_csc_b10_4_coeffs.dat')
     open(unit=60, file='../output_file/node_info_csc_b10_4_rows.dat')
  ...
  subroutine fill_node_array(node_arr_csr,grid_pty,grid_ptz,ny,nz,n,f,alpha_k)
...
contains
  ...
  if(PRECOND=="CLU")then
     ...
     call read_LU(K1,K2,n,read_file)
   ...
   call system_clock(start_count, count_rate, count_max)
     start_time = real(start_count)/real(count_rate)
  !Iteration start
  open(unit=10,file='../output_file/bicgstab_out_b10_4.dat')
  do i=1,2000
   if(PRECOND=="CLU")then
  ...
  close(20) 
  print*,"alpha=",alpha
  open(unit=40,file='../output_file/by_res_final_b10_4.dat')
  ...
  open(unit=60,file='../output_file/bz_res_final_b10_4.dat')
   write(60,*) ,'iter         y        z         by       res'
    do  j=0,ny
     do k=0,nz
        i=0.5*n + (k)*(ny+1)+j+1
        write(60,'(1X,I5,1X,7f10.6)') ,i,A_csr%y_loc(i),A_csr%z_loc(i),b(i),res(i)
     end do
       write(60,*) " "
    end do
  close(60)
...
PROGRAM  main
 ...
 real(kind=8),dimension(:),allocatable            :: q,b,act_b,error_b
 ...
 f=3*Rm/2/del_t
 n=(ny+1)*(nz+1)*2    !Multiplied by 2 because we are solving B (MAG FIELD) in y and z dirn. together
 init_by=0
 init_bz=0
 ...
 print *,'size of node_info' , size(node_info_csr%col)
